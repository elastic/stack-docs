...
same values. However, if your detectors use 
{ml-docs}/ml-metric-functions.html[metric] or 
{ml-docs}/ml-sum-functions.html[sum] analytical functions, the recommendation is 
to set the date_histogram or composite aggregation interval to a tenth of the 
bucket span. You get finer, more granular time buckets this way, which are ideal 

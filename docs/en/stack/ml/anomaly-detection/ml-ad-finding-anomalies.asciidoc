...
detector applies an analytical function to specific fields in your data. In
addition to limiting the number of detectors, the single metric job
...
A _categorization job_ groups log messages into categories and uses count or
rare functions to detect anomalies within them. See <<ml-configuring-categories>>.

...
existing snapshot repositories and snapshot details. It does not enable
users to change or restore any of them. It also does not enable users to change
index settings or to view or change their contents. The privileges granted by
this role are less than the _read index_ over all the indices because it does
not permit installing new repositories or restoring any snapshots.

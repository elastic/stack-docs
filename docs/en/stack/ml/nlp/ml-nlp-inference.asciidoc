...
When using deployed NLP models set the reindex `size` option to a value smaller
than the deployment's queue size. If reindex `size` is larger than the queue capacity
requests may be rejected with a too many requests 429 error code.
...
. The trained model is not deployed: Start the deployment with the {ref}/start-trained-model-deployment.html[start trained model deployment API].
. The default input field name is not present in the source document: Use the **Field Map** to map
the field name to the expected name.
. Too many requests: If using bulk ingest reduce the number of documents in the bulk request, for reindex
use the `size` parameter to decrease the number of documents processed in each batch.


[discrete]
[[ml-nlp-inference-handling-failures]]
== Handling failures
These common failure scenarios and others can be caputured by defining an `on_failure` clause
on the inference processor. Set the error message in the document from the special
`on_failure_message` field and re-route the document to a different _failed_ index for review later.

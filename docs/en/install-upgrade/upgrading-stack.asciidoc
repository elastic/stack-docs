...
compatible index structure. If you are upgrading from a version prior to
{prev-major-version}, you must upgrade to the most recent version of
{prev-major-version} first. Then you can use the {kib} Upgrade Assistant or
equivalent {ref}/migration-api-assistance.html[`_migration/assistance` API] to
check whether you need to upgrade the internal indices. 

...
* 6.3.0 and 6.3.1 might disable security for trial licenses. If you have a 6.2.x 
or earlier cluster with a trial license, {security} is disabled when you upgrade 
to 6.3.0 or 6.3.1. See https://www.elastic.co/blog/elastic-stack-6-3-0-and-6-3-1-may-disable-security-for-trial-licenses
* 2.x indices are not compatible with {version}. You must remove or reindex them 
on your 5.n cluster before upgrading to {version}. The internal Kibana and 
{xpack} indices and the default Beats and Logstash mapping templates also need 
to be updated to work with {version}.

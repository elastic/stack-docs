...
Under the anomaly timeline, there is a section that contains annotations. 
Annotations are notes that refer to events in a specific time period. They can 
be created by the user or generated automatically by the {anomaly-job} to 
reflect model changes and noteworthy occurrences. You can filter the type of 
events by using the selector on the right side of the **Annotations** section. 
You can also add your own annotations by drag-selecting a period of time in the 
**Single Metric Viewer** and adding a description.

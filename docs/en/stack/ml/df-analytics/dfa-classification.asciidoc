...
the classes that you want to predict (no more than 30 classes). We refer to this 
field as {depvar}. The rest of the fields that are included in the analysis are 
used to create the model for predicting the value of the {depvar} (these are the 
{feature-vars}). All the 
{ref}/put-dfanalytics.html#dfa-supported-fields[supported fields] are included 
by default, however, you can explicitly exclude and/or include fields. As the 
runtime and the necessary resources for the analysis are increasing as the 
number of included fields is growing,  

For more information about field selection, see 
...
Your training data set should be approximately balanced. It means the number of 
data points belonging to the various classes are not widely different. It is 
also necessary to have sufficient representation from all classes so that the 
...
and the characteristics of the data set. Complex decision boundaries – the rules 
that separate the different classes in a data set – are harder to learn and 
...
runtime of the analysis scales approximately linearly with the number of 
involved documents below roughly 200.000 data points. Therefore, if you double 
the number of documents, then the runtime of the analysis doubles respectively. 
Above 200.000 data points, runtime increases faster.

...
when the
<<detections-prerequisites, `xpack.encryptedSavedObjects.encryptionKey`>> 
...
NOTE: For *on-premises* {stack} deployments only, this message may be displayed 
when the <<detections-prerequisites, `xpack.security.enabled`>> setting is 

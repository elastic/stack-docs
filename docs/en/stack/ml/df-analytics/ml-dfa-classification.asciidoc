...
data point in a data set. Typical examples of {classification} problems are 
classifying malicious and benign domains to detect DGA activities for security 
reasons or predicting customer churn based on customer calling data. 
{classification-cap} is for predicting discrete, categorical values.
...
{ref}/evaluate-dfanalytics.html[evaluate {dfanalytics} API] or by viewing the job results in {kib}. The 
...
.Example 
You have three classes: `A`, `B`, and `C`, you want to calculate AUC ROC for 
`A`. In this case, the number of correctly classified ++A++s (true positives) 
are compared to the number of ++B++s and ++C++s that are misclassified as ++A++s 
(false positives).
...
deployed and used as functions. The <<ml-inference-class>> section shows you how 
...
boolean `FlightDelay` field. For an overview of these concepts, see 
<<ml-dfa-classification>> and <<ml-supervised-workflow>>.

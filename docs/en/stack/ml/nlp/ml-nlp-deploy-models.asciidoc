...
1. Download the model artifact files from https://ml-models.elastic.co/.
...
4. Verify that Nginx runs properly by visiting the following URL in your 
browser:
...
5. Point your Elasticsearch deployment to the model artifacts on the HTTP server
...
7. {ref}/restart-cluster.html#restart-cluster-rolling[Restart] the 
...
1. Download the model artifact files from https://ml-models.elastic.co/.
2. Put the files into a `models` subdirectory inside the `config` directory of 
your Elasticsearch deployment.
3. Point your Elasticsearch deployment to the model directory by adding the 
...
5. {ref}/restart-cluster.html#restart-cluster-rolling[Restart] the 
master-eligible nodes one by one.

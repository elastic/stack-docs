...
You can set additional deployment options - the number of allocations and the 
threads per allocation - that affect the performance of the model.

Model allocations are independent units of work for NLP tasks. Throughput can be 
scaled by adding more allocations to the deployment, it increases the number of 
{infer} requests that can be performed in parallel. Allocations are distributed 
to different {ml} nodes where possible. In some situations, however, multiple 
allocations may be assigned to the same node. Allocations on the same node share 
a copy of the model in memory but use a separate set of threads to evaluate the 
model. The number of allocations setting represents the amount of model 
allocations across all the {ml} nodes. Increasing the number of allocations 
generally increases the throughput.

NOTE: Model allocations are distributed in such a way that the total number of 
used threads does not exceed the allocated processors of a node.

The threads per allocation setting represents the number of threads used by each 
model allocation during {infer}. Increatsing the number of threads generally 

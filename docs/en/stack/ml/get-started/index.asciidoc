[[ml-getting-started]]
== Tutorial: Getting started with {ml}
...
environment, the `elastic` has the `superuser` role and you
could inadvertently make significant changes to the system. Therefore it's
wisest to play with this tutorial in a test or development environment. For more
information, see <<setup-privileges>>.

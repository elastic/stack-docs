...
When security is enabled with a Gold or Platinum license, 
TLS encryption must be configured for the {es} transport layer. 
...
See {ref}/bootstrap-elastic-passwords.html[Bootstrap Elastic passwords].
...
IMPORTANT: At this point, Kibana cannot connect to the {es} cluster.
You must generate a password for the built-in `kibana` user, update the `ELASTICSEARCH_PASSWORD` 
in the compose file, and restart to enable Kibana to communicate with the secured cluster. 
...
Specifying settings for {es} and {kibana} directly in the compose file is a convenient way to get started, 

...
[role="xpack"]
== Setting up infrastructure and logs monitoring

To set up infrastructure and logs monitoring, you need:
...
// I have no idea what to do with this section to split it for metrics and logs. I think eventually, I'm going to want to refer to a single
// location in the "integrations" section.
...
To populate the *Hosts* view with metrics and logs data, enable:
...
To populate the *Docker* view with metrics and logs data, enable:
...
To populate the *Kubernetes* view with metrics and logs data, enable:
...
If your metrics data or logs data has non-standard fields, you may need to modify some configuration settings in {kib}, such as the index pattern used to query the data, and the timestamp field used for sorting.
For more information, see {kibana-ref}/infrastructure-ui-settings-kb.html[{metrics} Settings] and {kibana-ref}/logs-ui-settings-kb.html[{logs} Settings].

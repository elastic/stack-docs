...
for creating signals. You can use the {kib} Alerts and Actions framework to
send notifications when signals are generated.
...
optimize the prebuilt rules, see <<tuning-detection-signals>>. Additionally,

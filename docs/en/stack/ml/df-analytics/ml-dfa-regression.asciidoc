...
{regression-cap} in the {stack} can be used to predict numerical data. (NEED TEXT)
...
data set that has some {feature-vars} and a {depvar}. You might also need to 
{ref}/transforms.html[{transform}] your data to create a {dataframe} which can 
be used as the source for {regression}.

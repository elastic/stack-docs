...
The different shapes represent different classes which are devided by the line 
(that represents the classification process carried out by the trained model) on 
the chart into two groups. The trained model enables you to predict the "shape" 
of new values.

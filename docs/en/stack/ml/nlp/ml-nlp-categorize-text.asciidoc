...
:description: NLP tasks that classify input text into categories and determine \
...
Text classification puts the input text into one of multiple classes or 
categories that best describe the text. The categories and their numbers depend 
on model training. Based on the number of classes, two main types of 
classification exist: binary – where the number of classes is two – and 
multi-class classification – where the number of classes is more than two.
...
This task allows you to use the same model to perform a difference inference, 
such as:
...
the same model and supplying different labels, a model trained on a specific set 
of labels is likely more accurate.
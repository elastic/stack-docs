...
=== External integrations using the {kib} Actions API
...
Call <<cases-find-connectors>> to retrieve connector IDs.
...
<<register-connector, created>> and <<assign-connector, assigned>> a connector.

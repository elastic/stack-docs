[[ml-nlp-ner-example]]
= How to deploy named entity recognition (NER)
...
This page demonstrates how to deploy a 
...
publically available on https://huggingface.co/[HuggingFace].
...
include::https://github.com/elastic/stack-docs/docs/en/stack/ml/shared-content.asciidoc[tag=nlp-requirements]


[discrete]
[[ex-ner-deploy]]
== Deploying a NER model

include::https://github.com/elastic/stack-docs/docs/en/stack/ml/shared-content.asciidoc[tag=nlp-eland-clone-docker-build]
...
include::https://github.com/elastic/stack-docs/docs/en/stack/ml/shared-content.asciidoc[tag=nlp-start]

include::https://github.com/elastic/stack-docs/docs/en/stack/ml/shared-content.asciidoc[tag=nlp-sync]
...
Deployed models can be evaluated using the 
{ref}/infer-trained-model-deployment.html[_infer API]. The input is the string 
you want to analyze. In the following request, `text_field` is the field name 
where the model expects to find the input, as defined in the model 
configuration. By default, if the model was uploaded via Eland, the input field 
is `text_field`.
...
"Mountain View" and "California" as the response shows:
...
[discrete]
[[ex-ner-ingest]]
== Adding the NER model to an {infer} ingest pipeline

You can perform bulk {infer} on documents as they are ingested via an ingest 
pipeline by using the {ref}/inference-processor.html[{infer} processor]. The 
novel _Les Misérables_ by Victor Hugo is used as an example for {infer} in 
the following. 
https://github.com/elastic/stack-docs/blob/main/docs/en/stack/ml/nlp/data/les-miserables-nd.json[Download] 
...
Now create an ingest pipeline either via the 
{ref}/ml-nlp-inference.html#ml-nlp-inference-processor[Stack management UI] or 
by using the API:
...
The `field_map` obejct of the `inference` processor maps the `paragraph` object 
(the field to analyze in the source documents) to `text_field` (the name of the 

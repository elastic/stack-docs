...
The example uses a public dataset from the 
...
generated answers for them. The example works with a sample of this dataset, 
...
The data set 
https://msmarco.blob.core.windows.net/msmarcoranking/msmarco-passagetest2019-top1000.tsv.gz[msmarco-passagetest2019-top1000.tsv] 
is a subset of the MS MACRO Passage Ranking data set used in the testing stage 
of the 2019 TREC Deep Learning Track. It contains 200 queries and for each query 
a list of relevant text passages extracted by a simple IR system. From that 
dataset, all unique passages with their IDs have been extracted and put into a 
...
where the ingest processor stores the embeddings. The used modelproduces 
embeddings as 384 dimensional vectors, hence you can use the indexed 
`dense_vector` field type with 384 dims:
...
Reindex the data from the `collection` index to the `collection-with-embeddings` 
index by pushing documents through the {infer} pipeline.
...
You can use the dense vector in the `collection-with-embeddings` index by doing 
...
As a result, you receive the top 10 closest to the query documents from the 
`collection-with-embedings` index sorted by their proximity to the query:

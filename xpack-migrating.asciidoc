...
This boolean setting has been replaced by `xpack.ssl.verification_mode` that supports more options as described in the
<<ssl-tls-settings, ssl settings>> reference section.
...
options as described in the <<ssl-tls-settings, ssl settings>> reference section.
...
include::shield/authentication/migrate-tool.asciidoc[]
...
<<monitoring-settings, {monitoring} settings>> and other X-Pack configuration
options.

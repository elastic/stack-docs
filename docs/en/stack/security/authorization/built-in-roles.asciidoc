...
to view their metadata. This role does not enable users to change index settings
or to view or change their contents. It enables users to view the configuration
of existing snapshot repositories and of the snapshot details; it does not
enable users to change or restore any of them. The privileges granted by this
role are less than the _read index_ over all the indices because it does not
permit installing new repositories or restoring any snapshots.

...
For the resource levels when adaptive resources are enabled, refer to <<nlp-model-adaptive-resources,this section>>.


[discrete]
[[nlp-model-auto-scale]]
=== NLP model autoscaling

You can enable autoscaling for each of your NLP model deployments.
...
include::ml-nlp-shared.asciidoc[tag=ml-nlp-adaptive-resources]

...
The Detection Engine page provides an overview of all the signals created by 
detection engine rules. It is also the place where you can enable prepackaged 
rules and create new rules. <<detection-engine-overview>> provides a detailed 
description of the detection engine and how to use it.

[role="screenshot"]
image::detection-engine.png[]

The `Signal detection rate` histograms show the detection rate of signals 
according to various attributes, including `Risk scores`, `Severities`, and 
`Top event categories`. The `All signals` table helps with investigations, allowing you to search, filter, and aggregate all {siem-soln} signals.

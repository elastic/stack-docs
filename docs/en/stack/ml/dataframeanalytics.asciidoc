...
{dfanalytics-cap} allows you to perform different analyses of your data and 
...
outliers – have different feature values and will therefore be far away from the 
normal points.

FIGURE ON DISTANCE BASED METHOD

The distance of K^th^ nearest neighbor method (`distance_kth_nn`) computes the 
distance of the data point to its K^th^ nearest neighbor where K is small number 
and usually independent of the total number of data points. The higher this 
distance the more the data point is an outlier.
...
account not only the distance of the points to their K nearest neighbours, but 

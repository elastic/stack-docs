...
service. You can use these services to exchange the current authentication for a
token or key, which is used for authentication without providing other means of
authentication. These services are enabled by default when TLS/SSL is enabled
for HTTP.

...
{kib} needs to know which SAML realm from the ones that are possibly configured
in {es} it should use, in order to initiate a SAML authentication. This can be accompliced
by setting the SAML realm name in Kibana explicitly, using the `xpack.security.authc.saml.reaml`
setting, which needs to match the name of the SAML realm that is configured in {es}.  

If you get an error like the one above, it possibly means that the value of 
`xpack.security.authc.saml.reaml` in your {kib} configuration  is wrong. Please 
verify that it  matches the name of the configured realm in {es} (this should 
be the string after `xpack.security.authc.realms.saml.` in your {es} configuration).

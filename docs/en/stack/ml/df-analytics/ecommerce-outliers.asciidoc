...
The goal of <<dfa-outlier-detection>> is to find the most unusual documents in
an index. Let's try to detect unusual customer behavior in the 
...
. Create a {dfanalytics-job} to detect outliers in the entity-centric index.
+
--
...
for Wagdi Shaw indicates that the sum of the product prices that was the most
...
from the new index. For example:

...
* Detects network activity caused by processes that occur rarely compared to 
  other processes (using the {ml-docs}/ml-functions.html#ml-rare[`rare` function]).
...
* Detects destination port activity that occurs rarely compared to other port 
  activities (using the {ml-docs}/ml-functions.html#ml-rare[`rare` function]).
...
* Detects listening port activity that occurs rarely compared to 
  other port activities (using the {ml-docs}/ml-functions.html#ml-rare[`rare` function]).
...
* Detects a web URL request that is rare compared to other web URL 
  requests (using the {ml-docs}/ml-functions.html#ml-rare[`rare` function]).
...
* Detects processes that occur rarely compared to other processes on all 
  hosts (using the {ml-docs}/ml-functions.html#ml-rare[`rare` function]).
...
* Detects users that are rarely or unusually active compared to other users 
  (using the {ml-docs}/ml-functions.html#ml-rare[`rare` function]).
...
* Detects unusually rare processes compared to other processes on the host (using the {ml-docs}/ml-functions.html#ml-rare[`rare` function]).
...
* Detects unusually high number of authentication attempts (using the 
  {ml-docs}/ml-functions.html#ml-nonzero-count[`high_non_zero_count` function]).
...
(using the {ml-docs}/ml-functions.html#ml-distinct-count[`high_distinct_count` function]).
...
before (using the {ml-docs}/ml-functions.html#ml-rare[`rare` function]).
...
the {ml-docs}/ml-functions.html#ml-rare[`rare` function]).
...
processes (using the
{ml-docs}/ml-functions.html#ml-rare[`rare` function]).
* Works on ECS compatible events across multiple indices.
...
{ml-docs}/ml-functions.html#ml-nonzero-count[`high_non_zero` function]).
* Works on ECS compatible events across multiple indices.
...
{ml-docs}/ml-functions.html#ml-count[`high_count` function]).
* Works on ECS compatible events across multiple indices.
...
{ml-docs}/ml-functions.html[`rare` function]).
* Works on ECS compatible events across multiple indices.
...
* Detects unusual DNS activity (using the 
  {ml-docs}/ml-functions.html#ml-info-content[`high_info_content` function]).
...
* Detects HTTP user agent activity that occurs rarely compared to other HTTP 
  user agent activities (using the {ml-docs}/ml-functions.html#ml-rare[`rare` function]).
...
* Analyzes host activity logs where `agent.type` is `winlogbeat`.
* Models occurrences of processes in paths.
* Detects activity in unusual paths (using the {ml-docs}/ml-functions.html#ml-rare[`rare` function]).
...
* Models occurrences of process creation activities (`partition_field_name` is 
  `process.parent.name`).
* Detects process relationships that are rare compared to other process 
  relationships (using the {ml-docs}/ml-functions.html#ml-rare[`rare` function]).
...
* Detects Windows service activities that occur rarely compared to other Windows service activities (using the {ml-docs}/ml-functions.html#ml-rare[`rare` function]).
...
* Detects user context switches that occur rarely compared to other user context switches (using the {ml-docs}/ml-functions.html#ml-rare[`rare` function]).
...
* Detects user remote login activities that occur rarely compared to other 
  user remote login activities (using the {ml-docs}/ml-functions.html#ml-rare[`rare` function]).

...
You can deploy a model multiple times by assigning a unique deployment ID to 
each deployment. It enables you to have dedicated deployments for different 
purposes, such as search and ingest. By doing so, you ensure that the search 
speed remains unaffected by ingest volume, and vice versa. Having separate 

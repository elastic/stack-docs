...
TIP: X-Pack Kibana provides an
{xpack-ref}/xpack-upgrade-assistant.html[Upgrade Assistant UI] to aid in the
upgrade process. You can upgrade the internal Kibana and X-Pack indices via
this utility.
...
Starting in 5.6 we have an Upgrade Assistant UI available in X-Pack Kibana.

===== Upgrading internal X-Pack indices for {major-version}
...
===== Upgrading Kibana indices for {major-version}

...
** If you are using Elastic Cloud, add `output.elasticsearch.pipeline: geoip-info`
to the Beats configuration files:
+
[source,yml]
----------------------------------
output.elasticsearch.pipeline: geoip-info <1>
----------------------------------
<1> The value of this field must be the same as the ingest pipeline name in
<<geo_pipeleine, step 1>> (`geoip-info` in this example).
+
** For local Elastic deployments, add `pipeline: geoip-info` under the

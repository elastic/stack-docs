...
{ml} node where your model is deployed. Every allocations on a node share the 
...
NOTE: If either the number of allocations or the threads per allocation value are 
...
these settings are greater than the number of hardware threads, the threads per 
allocation value is reduced automatically.

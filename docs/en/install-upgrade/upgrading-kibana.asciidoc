...
For more information, see {kibana-ref}/upgrade-migrations.html[upgrade migrations].
====
...
. To install from the `zip` or `tar.gz` archive:
...
IMPORTANT: {kib} has a new logging system in 8.0 and the {kib-ref}/logging-configuration-changes[log formats have changed]. 
For additional information, see {kib-ref}/configuring-logging.html[configuring logging].
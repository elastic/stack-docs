...
As we saw in previous steps, {kib} recognizes the sample data and has some
pre-configured jobs that it recommends. In the Machine Learning app, you can add
the jobs by selecting the *{kib} sample data web logs* configuration when you
are creating jobs or using the *Data Visualizer*. 

TIP: Alternatively, after you load a sample data set on the {kib} home page,
click *View data* > *ML jobs*. There are {ml} jobs for both the sample
eCommerce orders data set and the sample web logs data set.

[role="screenshot"]
image::ml/images/ml-gs-create-web-jobs-1.jpg["Create jobs for the sample web logs"]

The wizard recommends three jobs: `low_request_rate`, `response_code_rates`, and
`url_scanning`. You can optionally prefix those job identifiers with a name of
your choosing.
...
[role="screenshot"]
image::ml/images/ml-gs-create-web-jobs-2.jpg["Create jobs for the sample web logs"]
...
[role="screenshot"]
image::ml/images/ml-gs-web-results.jpg["Create jobs for the sample web logs"]
...
Before we delve deeper into those anomalies, let's take a step back and consider
just the simple, single metric job: `low_request_rate`.

[discrete]
[[ml-gs-single-metric-jobs]]
=== Single metric jobs

The `low_request_rate` job is a single metric job; it contains a single
_detector_. A detector defines the type of analysis that will occur
(in this case, a <<ml-count-functions,`low_count` analytical function>>) and the
fields that will be analyzed (in this case, none).

Consider, for example, that the sample data is access logs from your website.
You know that when your application is running well, there is typically a high
rate of requests. If the request rate drops significantly, it's an indication
that there might be problems on your website that merit investigation. 

Go to *Job Management* expand the details about the `low_request_rate` job.
Select the *Job config* tab:

[role="screenshot"]
image::ml/images/ml-gs-web-config1.jpg["Job configuration details for sample web logs"]


////
TO-DO: Show aggregation used by datafeed.

TIP: If you are using aggregated data, you can configure your job to use a
`summary_count_field_name`. The {ml} algorithms will make the best possible use
of summarized data in this case. For more information, see
...
.. For the **Aggregation**, select `Mean`. This value specifies the analysis
...
.. For the **Field**, select `taxful_total_price`. This value specifies the field that
...
.. For the **Bucket span**, enter `1h`. This value specifies the size of the
...
picker in the {kib} toolbar.
+
--
The data spans a period of time relative to the date that you downloaded
the sample. For example, if the earliest order dates is February 6, 2019 and the
latest is March 9, 2019, you can choose to initially analyze data up to the end
of February. Click **Absolute** and set the start and end dates. Once
...
**Bucket span** field. This is an experimental feature that you can use to help
determine an appropriate bucket span for your data. 
--

. Provide a name for the job, for example `mean-prices`. The job name must
...
--
[role="screenshot"]
image::ml/images/ml-gs-job1.jpg["A graph of the mean total prices over time"]
--

As the job is created, the graph is updated to give a visual representation of
the progress of {ml} as the data is processed. This view is only available
whilst the job is running.
...
click **Continue from ...**, where the date varies depending on when you stopped
your {dfeed}. Specify a search end time that aligns with the end of your sample
data set, then click **Start**. The date picker defaults to the latest
...
in real time. For this, start your {dfeed} and select **No end time**. To stop a {dfeed}, click *Stop {dfeed}* in the actions column.
...
products or locations? You can begin to troubleshoot these situations by

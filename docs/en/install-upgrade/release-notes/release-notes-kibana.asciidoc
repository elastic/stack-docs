...
Review the following information about the {kib} 9.0.0 release.
...
*Details* +
The button in Discover and Dashboards to create a new search session will no longer appear.

*Impact* +
This impacts all users.

*Action* +
To enable search sessions for all users, please set `data.search.sessions.enabled: true` in `kibana.yml`.
...
*Details* +
Users can no longer configure Discover to load fields from `_source`. This is an internal optimization and should have little impact on users. https://github.com/elastic/kibana/pull/202679
====

...
IMPORTANT: Don't start the {es} service yet! Additional configuration is needed.
...
. Specify the host URL where {agents} will reach {fleet-server}, for example: `https://10.128.0.203:8220`. This is the inet value that you copied from the `ifconfig` output.
+

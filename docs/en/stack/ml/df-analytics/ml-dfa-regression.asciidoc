...
want to use to create a model for predicting other fields. _Feature variables_ are the 
fields that are used to create the model. The _dependent variable_ is the field 
you want to predict.
...
This {ml} feature uses an ensemble learning technique that is a type of boosting 
...
testing (testing data set). Once the model is trained, you can let it 

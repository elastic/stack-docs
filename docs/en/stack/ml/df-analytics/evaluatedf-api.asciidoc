...
The multiclass confusion matrix contains the number of occurrences where the 
...
multiclass confusion matrix provides a summary of the performance of the 
...
It is a two by two matrix because there are only two classes. The matrix shows 
what proportion of the data points is correctly identified as members of a 
certain class (`true` and `false` in this case) and what proportion of them is 
misidentified.

The confusion matrix of a {classification} problem with numerous classes becomes 
more complicated as the number of classes grows.
...
filled in the table broken down for each class. This way you can examine the 
ways how the {classanalysis} confused the different classes while it made the 
predictions.

...
override the default behavior if you like. **Distance based methods** assume 
...
**Density based methods** are used for mitigating this problem.

The four algorithms don't always agree on which points are outliers. By default, 
we use all these methods during {oldetection}, then normalize and combine their 
...
{oldetection-cap} in the {stack} can be used to detect any unusual enitity in a 
...
destination index, merges them, and indexes them back to the destination index.

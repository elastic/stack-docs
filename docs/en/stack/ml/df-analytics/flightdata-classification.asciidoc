...
PUT _ml/data_frame/analytics/model-delayed-flights
{
  ...
  "dest": {
    "index": "df-flight-results",  <3>
    "results_field": "ml" 
  },
  "analysis": {
    "classification": {
      "dependent_variable": "FlightDelay",  <4>
      "training_percent": 70  <5>
    }
  },
  "analyzed_fields": {
    "includes": [],
...
----  
          ...
          "ml" : {
            ...
            "FlightDelay_prediction" : "false", <5>
            "is_training" : false <6>
          }
----
...
probability.
<3> The probability that the datapoint belongs to the given class. This is a 
value between 0 and 1. The higher the number the higher the probability that the 
datapoint belongs to the class.
<4> Name of the class.
<5> The prediction. The field name is suffixed with `_prediction`.
<6> Indicates that this document was not used in the training set.

The example above shows that the analysis has predicted the probability of all 
the possible classes. In this case, there are two classes: `true` and `false`. 
The class names along with the probability of the given classes are displayed in 
the `top_classes` object. The most probable class is the prediction. In the 
example above, `false` has a `class_probability` of 0.93 while `true` has only 
0.06, so the prediction will be `false` which coincides with the ground truth 
contained by the `FlightDelay` field. The class probability values help you to 
understand how sure the model is about the prediction. The higher number means 
that the model is more confident.
...
The API call returns a similar response:
...
--------------------------------------------------
<1> The name of the actual class, in this case, there are two actual classes: 
`true` and `false`.
...
The API response above shows that there are 912 documents in the dataset that 
have the `false` class. The model labelled 755 documents (out of 912) correctly 
as `false` and 157 documents as `true` while those are actually `false`. There 
are 306 documents in the dataset that have the `true` class. 80 of them are 
predicted as `false` and 226 are predicted correctly as `true`.
--
...
Text embedding allows for semantic embedding of text for dense information retrieval. The required format here is that 
the model outputs the specific embedding directly without any additional pooling. 
...
With text classification, the entire sequence is classified. An example of this is sentiment analysis. The output of 

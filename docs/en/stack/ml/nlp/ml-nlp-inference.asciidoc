...
TIP: When using deployed NLP models, set the reindex `size` option to a value
smaller than the deployment's queue size. Otherwise, requests might be rejected

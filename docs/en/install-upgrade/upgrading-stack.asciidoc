...
In 6.3 and later, {xpack} is automatically installed when you install {es},
{kib}, and Logstash. 
...
You can perform rolling upgrades to {version} from OSS-only clusters running
6.0-6.2. Basic features are operational once the cluster is fully upgraded. If
you are already using {xpack}, your settings are preserved when you upgrade.

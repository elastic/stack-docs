...
// tag::siem-jobs[]
ifeval::[{docname} == "ootb-ml-jobs"]
:rare-func-ref: <<ml-rare,`rare` function>> 
:nonzero-count-func-ref: <<ml-nonzero-count,`high_non_zero_count` function>>
:info-content-func-ref: <<ml-info-content,`high_info_content` function>>
endif::[]
ifeval::[{docname} != "ootb-ml-jobs"]
:rare-func-ref: {ml-docs}/ml-rare-functions.html#ml-rare[`rare` function] 
:nonzero-count-func-ref: {ml-docs}/ml-count-functions.html#ml-nonzero-count[`high_non_zero_count` function]
:info-content-func-ref: {ml-docs}/ml-info-functions.html#ml-info-content[`high_info_content` function]
endif::[]
[float]
...
* Detects network activity caused by processes that occur rarely compared to 
  other processes (using the {rare-func-ref}).
...
* Detects destination port activity that occurs rarely compared to other port 
  activities (using the {rare-func-ref}).
...
* Detects unusual listening port activity that occurs rarely compared to 
  other port activities (using the {rare-func-ref}).
...
* Detects unusual web URL request that is rare compared to other web URL 
  requests (using the {rare-func-ref}).
...
* Detects processes that occur rarely compared to other processes to all 
  Linux/Windows hosts (using the {rare-func-ref}).
...
* Detects HTTP user agent activity that occurs rarely compared to other HTTP 
  user agent activities (using the {rare-func-ref}).
...
* Detects activity in unusual paths (using the {rare-func-ref}).
...
* Detects process relationships that are rare compared to other process 
  relationships (using the {rare-func-ref}).
...
* Detects Windows service activities that occur rarely compared to other Windows 
  service activities (using the {rare-func-ref}).
...
* Detects user context switches that occur rarely compared to other user context 
  switches (using the {rare-func-ref}).
...
* Detects user remote login activities that occur rarely compared to other 
  user remote login activities (using the {rare-func-ref}).

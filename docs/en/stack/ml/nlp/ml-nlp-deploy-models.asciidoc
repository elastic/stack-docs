...
The number of allocations represents the amount of model allocations of each 
{ml} node where your model is deployed. Every allocation on a node share the 
same copy of the model in memory but use a separate set of threads to evaluate 
the model. Increasing the number of allocations generally increases the 
throughput.

The threads per allocation setting represents the number of threads used on each 
model allocation during {infer}. Increatsing the number of threads generally 
increases the speed of {infer} requests. 

NOTE: If either the number of allocations or the threads per allocation value is 
greater than the number of hardware threads, the value is automatically changed 
to a number that is less than the number of hardware threads. If the sum of 
the values of these settings are greater than the number of hardware threads, 
the threads per allocation value is reduced automatically.

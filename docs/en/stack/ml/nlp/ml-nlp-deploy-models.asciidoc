...
The number of allocations represents the amount of model allocations across all 
the {ml} nodes. Every allocation on a node share the same copy of the model in 
memory but use a separate set of threads to evaluate the model. Increasing the 
number of allocations generally increases the throughput.
...
used threads does not surpass the allocated processors of a node.

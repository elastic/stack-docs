...
Rollup indices and index patterns cannot be used in {ml} {dfeeds}. In {kib}, the
{ml} job creation wizards will not allow you to select an index pattern that uses
the rollup feature. If you select a saved search that uses a rollup index or add
a rollup index directly in the {dfeed} configuration JSON then the {ml} job
creation wizards will fail.

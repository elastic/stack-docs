...
Each {agent} can only be enrolled in a single policy.
...
A description of ingested data and its structure for each index.
...
A web-based UI in {kib} used to add and manage both {agent}s, and integrations for
...
A bundle of assets for configuring the ingestion and visualization of data,
or the protection of endpoints.
Any number of integrations can be added to an <<glossary-agent-policy,{agent} policy>>.
//Source: Observability

[[glossary-integration-policy]] integration policy::
A configuration definition for an input type.
<<glossary-agent-policy,{agent} policies>> consist of one or more integration policies.
//Source: Observability

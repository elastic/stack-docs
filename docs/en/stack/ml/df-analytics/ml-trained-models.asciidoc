...
information about this process, see <<ml-supervised-workflow>>, 
<<ml-inference-classification>> and <<ml-inference-regression>>.
...
* Note the query parameters that are used during export. These parameters are 
necessary to export the model in a way that it can later be imported again and 
used for inference.
...
URL, you can also set the model ID to something new using the last path part of 
the URL.
+
--
[source, bash]

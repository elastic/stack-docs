...
after <<map-links, defining field URLs>>, jump to specific Host and IP
details pages.
...
{kibana-ref}/field-formatters-string.html[format] these fields
as URL links:
...
. Edit the `host.name` field.

.. From the Format list, select _Url_.

.. From the Type list, select _Link_.
...
----------------------------------
<1> Where `<KibanaURL>` is the URL of your Kibana server.
...
. Repeat for the `source.ip` and `destination.ip` fields, using the following
Url template:
+
[source,url]
----------------------------------
...
. Repeat for all {siem-soln} indices. 

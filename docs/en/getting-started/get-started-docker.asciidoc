...
+
WARNING: Version {version} of {es} has not yet been released, so the
sample compose file is not yet available for this version.
endif::[]
...
+
WARNING: Version {version} of {es} has not yet been released, so the
sample compose and configuration files are not yet available for this version.
endif::[]

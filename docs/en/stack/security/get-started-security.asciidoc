...
. Generate a certificate and private key for each node in your cluster. In this 
tutorial, we are only using a single node cluster, 
...
The output from this command is a single PKCS#12 keystore that includes a node certificate, node key, and CA certificate. You can alternatively generate 
...
. Enable TLS in the {es} configuration file. For example, add the following 
setting in the `ES_PATH_CONF/elasticsearch.yml` file:
+
--
[source,yaml]
----
...
certificate. For example, add the following 
settings in the `ES_PATH_CONF/elasticsearch.yml` file, using the file and path 
names that you created in previous steps:
+
--
[source,yaml]
----
...
. If you specified a password when you ran the `elasticsearch-certutil cert` 
command, add that password to the {es} keystore.  For example: 
+
--
["source","sh",subs="attributes,callouts"]
----------------------------------------------------------------------
...
server and to connect to {es} via HTTPS. See
{kibana-ref}/using-kibana-with-security.html[Configuring Security in {kib}].

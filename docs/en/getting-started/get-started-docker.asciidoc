...
The `docker-compose.yml` file is a Docker Compose file that defines generates
node security certificates for {es}, configures a three-node {es} cluster with
TLS enabled for client and internode communications, and securely enrolls a {kib}
...
. Modify the `.env` file and enter a password value for the `ELASTIC_PASSWORD`
and `KIBANA_PASSWORD` variables.

. Create and start the three-node {es} cluster:
+
["source","sh"]
----
...
. Open a browser and navigate to http://localhost:5601[http://localhost:5601] to
...
[source,sh]
----
docker network rm <directory-name>_default
docker rm <directory-name>
----

`directory-name`:: Name of the directory that you created for the `.env` and
...
For example, to use `es-node01.yml` as the configuration file for the `es-node01`
{es} node, you can create a bind mount in the volumes section.
...
Similarly, to load {kib} settings from a file, you overwrite

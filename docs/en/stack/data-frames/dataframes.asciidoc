...
the aggregation is stored in a destination index, known as a {dataframe}. The 
...
{dataframe-transform} to run once (batch {dataframe-transform}) or continuously 
({cdataframe-transform}). A batch {dataframe-transform} is a single operation that 
will only reach checkpoint 1. {cdataframe-transforms-cap} continually 

...
By default, {es} includes a bundled Java Virtual Machine (JVM) supported by Elastic. While we strongly recommend using the bundled JVM in all installations of Elasticsearch, if you choose to use your own JVM, ensure it’s compatible by reviewing the Product and JVM support matrix. {es} 9.0+ requires Java 21 or later. 
...
NOTE: Unless noted, these following recommendations are best practices regardless of deployment method. 

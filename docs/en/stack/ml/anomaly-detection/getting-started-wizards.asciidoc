...
[role="screenshot"]
image::ml/images/ml-gs-data-keywords.jpg["Data Visualizer output for keyword and ip fields in {kib}"]
...
[role="screenshot"]
image::ml/images/ml-gs-data-geo.jpg["Data Visualizer output for geo_point fields in {kib}"]

For `text` fields, the **Data Visualizer** provides example values from the
selected time period. For example:

[role="screenshot"]
image::ml/images/ml-gs-data-text.jpg["Data Visualizer output for text fields in {kib}"]

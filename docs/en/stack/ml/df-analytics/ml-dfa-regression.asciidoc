...
tree learns from the mistakes of the previous one. Every tree is an iteration of 
the last one, hence it improves the decision made by the previous tree. The 
regression algorithm is optimized for a 
<<dfa-regression-lossfunction,loss function>> called mean-squared error loss by 
default.

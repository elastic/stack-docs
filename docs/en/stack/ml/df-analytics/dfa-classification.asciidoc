...
{stack}, you can perform both binary and multi-class {classanalysis}. The 
maximum number of classes for multi-class {classification} is 30.
...
case of multi-class {classification} an automated process called stratification 
...
is also necessary to have sufficient representation from all classes, so that 
the model can learn about each classes enough, otherwise the {classanalysis} may 
not provide the best predictions.
...
decision made by the previous tree. Both binary and multi-class {classification} 
mostly use the same machinery since multi-class is an extension of binary 
{classification}.
...
As a rule of thumb, multi-class {classification} takes more time to run than 
...
between these two factors is about linear. 
...
evaluation provides you is the confusion matrix for binary {classification} and 
multi-class confusion matrix for multi-class {classification}. These matrices 
tell you how many times a given data point that belongs to a given class was 
classified correctly and incorrectly. In other words, how many times your data 
point that belongs to the X class was mistakenly classified as Y or Z or any 
other class in the dataset other than X.

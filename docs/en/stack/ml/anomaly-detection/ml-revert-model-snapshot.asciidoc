...
. Optional: Select if you want to replay the analysis based on the data in your 
  index after the revert has been applied. You can decide whether you want the 
  job running in real time. You can also define the time range you don't want 
  the job to analyze, this way you can skip the time of the event you don't want 
  to impact the model behavior. If you don't select this option, there will be 
  no {anomaly-detect} results after the snapshot was taken.
+
--
[role="screenshot"]

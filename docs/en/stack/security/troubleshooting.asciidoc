...
. This can be resolved by correcting your `sp.acs` setting in your {es} SAML
realm configuration or by using the `xpack.security.public.*` settings in `kibana.yml`.
{kib} either uses the `server.host` and `server.port` or the `xpack.security.public.*`
settings to construct the SAML ACS URL and this URL needs to match exactly the URL that
is configured in the SAML realm configuration.

NOTE: The two URLs are compared as case-sensitive strings and not as canonicalized URLs.

. We could not find the metadata for the SAML Entity ID `your:entity.id` in the metadata
file (`metadata.xml`) that has been configured. Either the value of `idp.entity_id` setting
in the {es} SAML realm configuration is wrong/misspelled, or the wrong metadata file has been
configured in `idp.metadata.path`.
...
_actual_ problem one needs to check the Elasticsearch logs for further details.
...
can be a different error identifier than the one above and might offer an indication about
the cause of the failure, but `Message` and `Detail` will be `null`. In order to further
diagnose the problem please consult the logs and documentation of the Identity Provider.
...
.. The configuration in the Identity Provider has changed or the key has been rotated and
...
if the correct key is used.

. If you want your users to be able to use local credentials to authenticate to Kibana in
...
Very detailed trace logging can be enabled specifically for the SAML realm by adding the
following lines to the end of the `log4j2.properties` configuration file in the `ES_PATH_CONF`:
